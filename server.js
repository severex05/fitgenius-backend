...
// ===== Rota: ANALISAR IMAGEM DA REFEIÇÃO (PROMPT MELHORADO) =====
...
app.post('/api/analyze-image', upload.single('image'), async (req, res) => {
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'Você é um assistente nutricional especializado em análise de imagens de refeições. Sua tarefa é identificar os alimentos presentes, estimar suas quantidades em gramas e calcular os macronutrientes (calorias, proteínas, carboidratos, gorduras) para cada item. Responda APENAS com um JSON válido, sem nenhum texto adicional ou markdown. O JSON deve conter uma chave "alimentos" que é um array de objetos. Cada objeto no array deve ter as chaves "nome" (string), "quantidade" (number, em gramas), "calorias" (number), "proteinas" (number), "carboidratos" (number), "gorduras" (number). Se nenhum alimento for detectado ou a imagem não for de comida, retorne um array vazio em "alimentos".',
          },
          {
            role: 'user',
            content: [
              { type: 'text', text: 'Analise esta imagem de refeição e liste os alimentos com suas quantidades e macronutrientes.' },
              {
                type: 'image_url',
    ...
    // Garante sempre pelo menos a estrutura { alimentos: [] }
